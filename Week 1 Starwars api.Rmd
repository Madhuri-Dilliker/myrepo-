...
library(plyr) #joining the dataframes
```
...
```{r}
Planet_data$PlanetId <- gsub("[^0-9]", "", Planet_data$url)
```
...
```
12] Get the matching planet name based on planet id present in people's dataframe
