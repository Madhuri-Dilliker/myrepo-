...
Films_data
```
Viewing the Planet_data, People_data and Film_data dataframes
...
In People_data dataframe we want a new column called planet id which will store the id of Planet_data dataframe so id is extracted from the homeworld column which has planet endpoint api mentioned in the column.
...
In Planet_data create a new column to store the extract id from url column.

```{r}
Planet_data$Id <- gsub("[^0-9]", "", Planet_data$url)
```

First two rows in the people dataframe id are invalid so replacing it with NA values
...
First two rows in the planet dataframe id are invalid so replacing it with NA values
